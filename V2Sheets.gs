...
function hotelDbV2GetHeaderMap_(sheet) {
  ...
  Object.keys(HOTEL_DB_V2_CONFIG.SOURCE_ALIASES).forEach(function(key) {
    const aliases = HOTEL_DB_V2_CONFIG.SOURCE_ALIASES[key].map(
      hotelDbV2NormalizeText_
    );

    for (let column = 0; column < headers.length; column++) {
      if (
        aliases.indexOf(
          hotelDbV2NormalizeText_(headers[column])
        ) !== -1
      ) {
        map[key] = column + 1;
        break;
...
function hotelDbV2ValidateSourceSheet_(sheet, map) {
  ...
  if (reserved.indexOf(sheet.getName()) !== -1) {
    throw new Error(
      '出力用シートでは実行できません。元データのシートを開いてください。'
    );
  }

  if (!map.facilityName) {
    throw new Error('「施設名」列が見つかりません。');
  }
...
function hotelDbV2EnsureOutputHeaders_(sheet) {
  ...
  if (missing.length) {
    sheet
      .getRange(1, currentLastColumn + 1, 1, missing.length)
      .setValues([missing]);
  }
...
function hotelDbV2ReadFacility_(sheet, rowNumber, map) {
  ...
  function value(key) {
    return map[key]
      ? hotelDbV2Clean_(values[map[key] - 1])
      : '';
  }
...
function hotelDbV2BuildGoogleData_(place, score, facility, decision) {
  ...
  return {
    ...
    rating: place && place.rating !== undefined
      ? place.rating
      : '',
    reviewCount: place && place.userRatingCount !== undefined
      ? place.userRatingCount
      : '',
    businessStatus: hotelDbV2TranslateBusinessStatus_(
      place && place.businessStatus
    ),
    ...
    proposedMunicipality: hotelDbV2GetMunicipalityForSource_(
      place,
      facility.municipality
    ),
    proposedAddress: hotelDbV2GetAddressForSource_(
      place,
      facility
    ),
...
function hotelDbV2WriteGoogleData_(
  sheet,
  rowNumber,
  map,
  googleData,
  ...
  if (includePlaceId) {
    updates.placeId = googleData.placeId;
  }

  Object.keys(updates).forEach(function(key) {
    if (map[key] && updates[key] !== undefined) {
      sheet
        .getRange(rowNumber, map[key])
...
function hotelDbV2GetOrCreateSheet_(spreadsheet, name, headers) {
  ...
  if (sheet.getLastRow() === 0) {
    ...
    if (
      ...
      hotelDbV2Clean_(existing[0]) !== headers[0]
    ) {
      sheet
        .getRange(1, 1, 1, headers.length)
        .setValues([headers]);
...
function hotelDbV2Upsert_(context, key, object, preserveState) {
  ...
  const values = hotelDbV2RowFromObject_(
    context.headers,
    object
  );
  ...
  if (existingRow) {
    if (preserveState && context.map['状態']) {
      ...
      const oldValues = context.sheet
        ...
        .getDisplayValues()[0];
      const oldState = hotelDbV2Clean_(
        oldValues[stateColumn - 1]
      );
      const ignored = {
        '状態': true,
        '確認日': true,
      ...
      const unchanged = context.headers.every(
        function(header, index) {
          if (ignored[header]) return true;

          return hotelDbV2Clean_(oldValues[index]) ===
            hotelDbV2Clean_(values[index]);
        }
      );

      values[stateColumn - 1] =
        unchanged && oldState
          ? oldState
          : '未確認';
    }
...
function hotelDbV2AppendHistory_(context, object) {
  context.history.appendRow(
    hotelDbV2RowFromObject_(
      HOTEL_DB_V2_HISTORY_HEADERS,
      object
    )
  );
...
function hotelDbV2CompareFacility_(facility, googleData) {
  ...
  const sourcePostal = hotelDbV2NormalizePostalCode_(
    facility.postalCode
  );
  const proposedPostal = hotelDbV2NormalizePostalCode_(
    googleData.proposedPostalCode
  );
  ...
  const sourceMunicipality = hotelDbV2NormalizeText_(
    facility.municipality
  );
  const proposedMunicipality = hotelDbV2NormalizeText_(
    googleData.proposedMunicipality
  );

  if (
    proposedMunicipality &&
    sourceMunicipality !== proposedMunicipality
  ) {
    differences.push('市区町村');
  }

  const addressesEquivalent = hotelDbV2AddressesEquivalent_(
    facility.address,
    googleData.proposedAddress,
    facility.name,
    googleData.proposedName
  );
  ...
  const sourceName = hotelDbV2NormalizeText_(
    facility.name
  );
  const proposedName = hotelDbV2NormalizeText_(
    googleData.proposedName
  );
...
function hotelDbV2CorrectionCandidate_(
  sheet,
  facility,
  googleData,
  differences
...
function hotelDbV2ReviewCandidate_(
  sheet,
  facility,
  reason,
  candidate,
    ...
    object: {
      ...
      '一致スコア':
        score === undefined || score === null
          ? ''
          : score,
      '営業状態': hotelDbV2TranslateBusinessStatus_(
        place.businessStatus
      ),
      'Google Maps URL': hotelDbV2Clean_(
        place.googleMapsUri
      ),
      '確認日': hotelDbV2Today_(),
