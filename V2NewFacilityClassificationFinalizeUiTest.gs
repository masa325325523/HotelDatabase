...
function setupHotelDbV2NewFacilityClassificationUiTest() {
  ...
  try {
    if (
      ...
      throw new Error('PR #18 UIテストの残骸があります。先に cleanupHotelDbV2NewFacilityClassificationUiTest を実行してください。');
    }

    const realNewCandidates = spreadsheet.getSheetByName(
      HOTEL_DB_V2_NEW_FACILITY_CLASSIFICATION.SOURCE_CANDIDATE_SHEET
    );
    if (!realNewCandidates) {
    ...
    const requiredCandidateHeaders = [
      ...
      '候補Place ID', '候補施設名', '候補住所', 'Googleタイプ',
      '追加先シート', '追加先行'
    ];
    const realMap = {};
    realHeaders.forEach(function(header, index) {
      const text = hotelDbV2Clean_(header);
      if (text && !realMap[text]) realMap[text] = index + 1;
    });
    const missing = requiredCandidateHeaders.filter(function(header) {
      return !realMap[header];
    });
    if (missing.length) {
      throw new Error('通常の「新規追加候補」にPR18テストで必要な列がありません: ' + missing.join('、'));
    }
    ...
    const sourceCases = [
      {
        ...
        postal: '100-0001', municipality: '東京都千代田区',
        address: '東京都千代田区千代田1-1', name: 'PR18正常ホテル',
        googleType: 'hotel,lodging'
      },
      {
        placeId: HOTEL_DB_V2_PR18_UI_TEST.PLACE_IDS.UNAPPROVED,
        postal: '100-0002', municipality: '東京都千代田区',
        address: '東京都千代田区皇居外苑1-1', name: 'PR18未承認ゲストハウス',
        googleType: 'guest_house,lodging'
      },
      {
        placeId: HOTEL_DB_V2_PR18_UI_TEST.PLACE_IDS.CATEGORY_CHANGED,
        postal: '100-0003', municipality: '東京都千代田区',
        address: '東京都千代田区一ツ橋1-1', name: 'PR18分類変更旅館',
        googleType: 'japanese_inn,lodging'
      },
      {
        placeId: HOTEL_DB_V2_PR18_UI_TEST.PLACE_IDS.NOTES_CHANGED,
        postal: '100-0004', municipality: '東京都千代田区',
        address: '東京都千代田区大手町1-1', name: 'PR18備考変更民泊',
    ...
    const sourceRows = sourceCases.map(function(item) {
      ...
      set_('postalCode', item.postal);
      set_('municipality', item.municipality);
      set_('address', item.address);
      set_('facilityName', item.name);
      set_('category', '');
      set_('notes', 'PR17承認新規追加／宿泊分類要確認／PR18 UIテスト');
      set_('placeId', item.placeId);
      set_('googleName', item.name);
      set_('googleAddress', item.address);
      set_('businessStatus', '営業中');
      set_('matchDecision', '新規追加（承認）');
      return row;
    });
    sourceSheet.getRange(2, 1, sourceRows.length, sourceHeaders.length).setValues(sourceRows);

    testNewCandidateSheet = spreadsheet.insertSheet(
      HOTEL_DB_V2_NEW_FACILITY_CLASSIFICATION.SOURCE_CANDIDATE_SHEET
    );
    testNewCandidateSheet.getRange(1, 1, 1, realHeaders.length).setValues([realHeaders]);
    const testMap = {};
    realHeaders.forEach(function(header, index) {
      const text = hotelDbV2Clean_(header);
      if (text && !testMap[text]) testMap[text] = index + 1;
    });

    const candidateRows = sourceCases.map(function(item, index) {
      const row = new Array(realHeaders.length).fill('');
      function set_(header, value) {
        if (testMap[header]) row[testMap[header] - 1] = value;
      }
      const sourceRow = index + 2;
      set_('候補キー', 'PR18_UI_' + (index + 1));
      set_('状態', HOTEL_DB_V2_APPROVED_NEW_FACILITY.APPLIED_STATE);
      set_('探索元シート', sourceSheet.getName());
      set_('探索元シートID', String(sourceSheet.getSheetId()));
      set_('候補Place ID', item.placeId);
      set_('候補施設名', item.name);
      set_('候補住所', item.address);
      set_('候補郵便番号', item.postal);
      set_('候補市区町村', item.municipality);
      set_('営業状態', '営業中');
      set_('Googleタイプ', item.googleType);
      set_('追加先シート', sourceSheet.getName());
      set_('追加先行', String(sourceRow));
      set_('追加結果', '追加済み。宿泊分類要確認');
      return row;
    });
    testNewCandidateSheet
      .getRange(2, 1, candidateRows.length, realHeaders.length)
    ...
    try {
      ...
      const backupNewCandidates = spreadsheet.getSheetByName(HOTEL_DB_V2_PR18_UI_TEST.BACKUP_NEW_CANDIDATES);
      const backupHistory = spreadsheet.getSheetByName(HOTEL_DB_V2_PR18_UI_TEST.BACKUP_HISTORY);
      const backupClassification = spreadsheet.getSheetByName(HOTEL_DB_V2_PR18_UI_TEST.BACKUP_CLASSIFICATION);
      if (backedUpNewCandidates && backupNewCandidates) {
        backupNewCandidates.setName(HOTEL_DB_V2_NEW_FACILITY_CLASSIFICATION.SOURCE_CANDIDATE_SHEET);
      }
...
function prepareHotelDbV2NewFacilityClassificationApprovalUiTest() {
  ...
  const candidateSheet = spreadsheet.getSheetByName(HOTEL_DB_V2_NEW_FACILITY_CLASSIFICATION.SHEET_NAME);
  if (!sourceSheet || !candidateSheet) {
    throw new Error('PR #18 UIテスト用シートがありません。先にセットアップ→⑮を実行してください。');
  }

  const map = hotelDbV2NewFacilityClassificationHeaderMap_(candidateSheet);
  const required = ['状態', 'Place ID', '参考分類', '確定宿泊分類', '確定備考'];
  const missing = required.filter(function(header) { return !map[header]; });
  if (missing.length) throw new Error('「新規施設分類候補」に必要な列がありません: ' + missing.join('、'));
  ...
  [ids.NORMAL, ids.UNAPPROVED, ids.CATEGORY_CHANGED, ids.NOTES_CHANGED].forEach(function(id) {
    if (!rowByPlaceId[id]) throw new Error('⑮で作成されるはずの分類候補が不足しています: ' + id);
  });
...
function verifyHotelDbV2NewFacilityClassificationUiTest() {
  ...
  if (!sourceSheet || !candidateSheet || !historySheet) {
    throw new Error('PR #18 UIテスト用シートがありません。セットアップから順番に実行してください。');
  }
  ...
  function cv_(placeId, header) {
    const row = rowByPlaceId[placeId];
    return row && map[header] ? hotelDbV2Clean_(row[map[header] - 1]) : '';
  }

  const ids = HOTEL_DB_V2_PR18_UI_TEST.PLACE_IDS;
  check_('正常承認は反映済み', cv_(ids.NORMAL, '状態') === '反映済み', cv_(ids.NORMAL, '状態'));
  check_('未承認は未確認のまま', cv_(ids.UNAPPROVED, '状態') === '未確認', cv_(ids.UNAPPROVED, '状態'));
  check_('元分類変更は要再確認', cv_(ids.CATEGORY_CHANGED, '状態') === '要再確認', cv_(ids.CATEGORY_CHANGED, '状態'));
  check_('元備考変更は要再確認', cv_(ids.NOTES_CHANGED, '状態') === '要再確認', cv_(ids.NOTES_CHANGED, '状態'));
  check_('正常承認に反映日時あり', !!cv_(ids.NORMAL, '反映日時'));
  check_('正常承認の反映結果', cv_(ids.NORMAL, '反映結果') === '反映済み', cv_(ids.NORMAL, '反映結果'));
  check_('分類変更は未反映記録', cv_(ids.CATEGORY_CHANGED, '反映結果').indexOf('未反映') !== -1, cv_(ids.CATEGORY_CHANGED, '反映結果'));
  check_('備考変更は未反映記録', cv_(ids.NOTES_CHANGED, '反映結果').indexOf('未反映') !== -1, cv_(ids.NOTES_CHANGED, '反映結果'));

  const sourceMap = hotelDbV2GetHeaderMap_(sourceSheet);
  const sourceValues = sourceSheet.getRange(2, 1, 4, sourceSheet.getLastColumn()).getDisplayValues();
  function sv_(index, key) {
    return sourceMap[key] ? hotelDbV2Clean_(sourceValues[index][sourceMap[key] - 1]) : '';
  }
  check_('正常行だけホテル営業へ確定', sv_(0, 'category') === 'ホテル営業', sv_(0, 'category'));
  check_('正常行の要確認マーカー除去', sv_(0, 'notes').indexOf('宿泊分類要確認') === -1, sv_(0, 'notes'));
  check_('正常行の既存備考保持', sv_(0, 'notes').indexOf('PR17承認新規追加') !== -1, sv_(0, 'notes'));
  check_('正常行に確認済み追記', sv_(0, 'notes').indexOf('宿泊分類確認済') !== -1, sv_(0, 'notes'));
  check_('正常行に確定備考追記', sv_(0, 'notes').indexOf('郵便番号一致') !== -1, sv_(0, 'notes'));
  check_('未承認行は分類未入力', sv_(1, 'category') === '', sv_(1, 'category'));
  check_('未承認行は要確認マーカー保持', sv_(1, 'notes').indexOf('宿泊分類要確認') !== -1, sv_(1, 'notes'));
  check_('分類変更行を上書きしない', sv_(2, 'category') === '旅館営業', sv_(2, 'category'));
  check_('備考変更行の分類は未入力', sv_(3, 'category') === '', sv_(3, 'category'));
  check_('備考変更行の手修正を保持', sv_(3, 'notes').indexOf('手修正') !== -1, sv_(3, 'notes'));
  ...
  if (historyCount === 1) {
    ...
    const historyRow = historySheet.getRange(2, 1, 1, historySheet.getLastColumn()).getDisplayValues()[0];
    check_('履歴の処理名', historyMap['処理'] && hotelDbV2Clean_(historyRow[historyMap['処理'] - 1]) === '新規施設分類確定');
  } else {
    check_('履歴の処理名', false, '履歴件数が1件ではありません');
  }

  check_('通常の新規追加候補バックアップ保持', !!spreadsheet.getSheetByName(HOTEL_DB_V2_PR18_UI_TEST.BACKUP_NEW_CANDIDATES));
  check_('分類候補はテスト4件のみ', Object.keys(rowByPlaceId).length === 4, String(Object.keys(rowByPlaceId).length));
...
function cleanupHotelDbV2NewFacilityClassificationUiTest() {
  ...
  try {
    ...
    if (backupNewCandidates && testNewCandidates) spreadsheet.deleteSheet(testNewCandidates);
    if (backupHistory && testHistory) spreadsheet.deleteSheet(testHistory);
    if (backupClassification && testClassification) spreadsheet.deleteSheet(testClassification);
    if (!backupClassification && sourceSheet && testClassification) spreadsheet.deleteSheet(testClassification);
    if (!backupHistory && sourceSheet && testHistory) spreadsheet.deleteSheet(testHistory);
    if (sourceSheet) spreadsheet.deleteSheet(sourceSheet);
    ...
    SpreadsheetApp.getUi().alert([
      ...
      '通常の新規施設分類候補: ' + (backupClassification ? '復元済み' : '元々なし')
    ].join('\n'));
