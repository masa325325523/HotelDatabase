...
function runHotelDbV2HealthCheckTests() {
  ...
  configChecks.forEach(function(item) { check('CFG-' + item.id, item.ok === true, item.detail); });
  ...
  check('ENTRY-00 total 22', entries.length === 22, 'actual=' + entries.length);
  entries.forEach(function(item) { check(item.id + ' exists', item.exists === true, item.label); });
  ...
  check('REG-00 total 8', regressions.length === 8, 'actual=' + regressions.length);
  regressions.forEach(function(item) { check(item.id + ' exists', item.exists === true, item.label); });
  check('REG-unique', new Set(regressions.map(function(item){ return item.id; })).size === regressions.length);

  let summary = hotelDbV2HealthCheckSummarize_([{severity:'ok'}, {severity:'ok'}, {severity:'info'}]);
  ...
  summary = hotelDbV2HealthCheckSummarize_([{severity:'ok'}, {severity:'warning'}]);
  check('SUM-05 warning overall', summary.overall === '要確認');
  check('SUM-06 warning1', summary.counts.warning === 1);

  summary = hotelDbV2HealthCheckSummarize_([{severity:'warning'}, {severity:'critical'}]);
  ...
  const sections = hotelDbV2HealthCheckGroupSections_([
    {section:'安全設定', id:'1'}, {section:'セットアップ', id:'2'}, {section:'監査・保護', id:'3'}
  ]);
  ...
  const message = [
    ...
    'Google Places API呼出: なし', 'Script Properties変更: なし', '元DB・候補・履歴の変更: なし'
  ];
  if (failures.length) message.push('', '失敗:', failures.join('\n'));
  try { SpreadsheetApp.getUi().alert(message.join('\n')); } catch (ignore) {}
  if (failures.length) throw new Error('PR #23 自己診断で失敗があります: ' + failures.join(' / '));
