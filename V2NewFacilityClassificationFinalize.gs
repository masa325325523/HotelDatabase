 ...
 * - 候補再作成でスナップショットが変わった場合、以前の承認を引き継がない。
 * - 反映成功時だけ修正履歴へ記録する。
 */
...
function hotelDbV2NewFacilityClassificationUpsert_(sheet, map, index, data) {
  ...
  if (existingRow) {
    ...
    const criticalHeaders = [
      '元シート', '元シートID', '元行', 'Place ID', '施設名', '住所',
      '現在宿泊分類', '現在備考', 'Googleタイプ'
    ];
    const unchanged = criticalHeaders.every(function(header) {
      return hotelDbV2Clean_(old[map[header] - 1]) === hotelDbV2Clean_(row[map[header] - 1]);
    });

    if (data.state === '未確認' && unchanged && (oldState === '未確認' || oldState === '承認')) {
      row[map['状態'] - 1] = oldState;
      row[map['確定宿泊分類'] - 1] = old[map['確定宿泊分類'] - 1];
      row[map['確定備考'] - 1] = old[map['確定備考'] - 1];
    }

    if (oldState === '反映済み' && unchanged) {
      row[map['状態'] - 1] = oldState;
      row[map['確定宿泊分類'] - 1] = old[map['確定宿泊分類'] - 1];
      row[map['確定備考'] - 1] = old[map['確定備考'] - 1];
...
function hotelDbV2NewFacilityClassificationRecommend_(googleTypes) {
  ...
  if (text.indexOf('budget_japanese_inn') !== -1 || text.indexOf('hostel') !== -1 || text.indexOf('guest_house') !== -1 || text.indexOf('bed_and_breakfast') !== -1) {
    return { reference: '簡易宿所系（要確認）', reason: 'ゲストハウス・ホステル・B&B・簡易旅館等のGoogleタイプです。許可区分は別途確認が必要です。', confidence: 82 };
  }
  ...
  values.forEach(function(row, offset) {
    ...
    try {
      ...
      if (!sourceSheet || sourceSheet.getName() !== candidate.sheetName) {
        hotelDbV2NewFacilityClassificationSetResult_(sheet, map, rowNumber, HOTEL_DB_V2_NEW_FACILITY_CLASSIFICATION.CONFLICT_STATE, '未反映: 元シートが見つからないか、シート名が候補作成時から変更されています。');
        result.conflicts++;
        return;
      }
      ...
      if (!sourceMap.category || !sourceMap.notes) {
        hotelDbV2NewFacilityClassificationSetResult_(sheet, map, rowNumber, HOTEL_DB_V2_NEW_FACILITY_CLASSIFICATION.CONFLICT_STATE, '未反映: 元シートに「宿泊分類」または「備考」列がありません。');
        result.conflicts++;
        return;
      }
      if (candidate.sourceRow < 2 || candidate.sourceRow > sourceSheet.getLastRow()) {
        hotelDbV2NewFacilityClassificationSetResult_(sheet, map, rowNumber, HOTEL_DB_V2_NEW_FACILITY_CLASSIFICATION.CONFLICT_STATE, '未反映: 元行が見つかりません。');
        result.conflicts++;
        return;
      }
      ...
      if (didWrite && sourceSheet && candidate.sourceRow >= 2) {
        try {
